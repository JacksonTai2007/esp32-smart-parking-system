...
// ---------------------------------------------------------------
// 默认关闭刷卡：车辆被检测到即自动开闸（流程见 ParkingStateMachine）。
// 置 1 可恢复 RC522 刷卡鉴权（代码保留，0/1 两种配置均可编译）。
...
// Web 手动控制接口（POST /api/gate/open、/api/gate/close、/api/reset）。
// 安全边界：这些接口没有认证，同一网络内任何客户端都能触发物理闸机，
// 仅限本地演示网络使用，绝不可暴露到公共网络；置 0 后接口返回 403，
// 仪表盘只剩只读状态展示。
#define ENABLE_WEB_MANUAL_GATE_CONTROL 1
