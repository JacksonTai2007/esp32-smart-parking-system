...
// 出入口道闸（SG90 舵机）。开启后：车辆进 / 出场自动抬杆，保持数秒自动落杆；
// 网页另有「开闸 / 落闸」手动按钮。未接舵机也能开（LEDC 向空脚输出，无害），
// 网页与 OLED 仍会显示道闸状态，方便录视频。不需要道闸时置 0（退化为空实现）。
#define ENABLE_GATE          1
...
// ---------------------------------------------------------------
// 出入口道闸（SG90 舵机，LEDC PWM 驱动）
// SG90 转角范围 0~180°，本机用两个角度表示落杆 / 抬杆。实测若方向相反，
// 把两个角度对调即可（落杆≈水平挡车，抬杆≈竖起放行）。
// ---------------------------------------------------------------
constexpr uint8_t  GATE_SERVO_CLOSED_DEG = 10;    // 落杆（挡车）角度
constexpr uint8_t  GATE_SERVO_OPEN_DEG   = 95;    // 抬杆（放行）角度
constexpr uint32_t GATE_OPEN_HOLD_MS     = 3000;  // 自动抬杆后保持时长，到点自动落杆
