...
9. **火焰传感器 / 风扇继电器的触发电平**都在 `config/Settings.h` 可调
   （`FLAME_ACTIVE_LEVEL`、`FAN_ACTIVE_LEVEL`）。火焰模块常见"检测到火焰输出
   LOW"。火焰与触摸两路输入都启用了内部上/下拉，**不接模块也不会误报警/
   误入场**。课堂演示可用红外遥控器对准火焰传感器按键安全触发火警
   （火焰传感器本质是红外光敏管），无需明火。
