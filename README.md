...
```
车辆驶入车位 → 红外检测占用 → 识别入场并开始计时（蜂鸣 1 短响）
→ 车辆驶离 → 按停留时长结算费用（蜂鸣 2 短响）
→ OLED / Web 显示本次费用、累计收入与最近停车记录
```

> 说明：本期为控制复杂度与工期，**去掉了 RFID 刷卡鉴权与 SG90 闸机、HC-SR04 入口
> 超声波**，聚焦"车位识别 + 车位管理 + 按时长计费"这一最小闭环。RFID / 闸机
> 可作为 Phase 2 进阶项重新加入（见第 14 节）。
...
本期无舵机/电机等大电流执行器，USB 5V 直接供电即可，无需外接电源。
...
计费规则：停留时长 ≤ 免费时长 → 0；否则 应付分钟 = ⌈停留分钟⌉，
费用 = 应付分钟 × 单价。金额全程用整数"分"运算，避免浮点误差。
...
- 出入口闸机（SG90 / 28BYJ-48）与 RFID 会员/月卡识别，组成完整出入场闭环
