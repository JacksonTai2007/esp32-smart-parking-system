...
> 道闸**（进出场自动抬杆、网页手动开 / 落，可由 `ENABLE_GATE` 关闭）。**RFID 刷卡鉴权
> 与 HC-SR04 入口超声波**仍未纳入，可作为 Phase 2 进阶项（见第 14 节）。
...
RFID 会员识别、道闸与刷卡鉴权联动、数据持久化与计费记录导出等。
...
- RFID 会员/月卡识别，与现有 SG90 道闸联动做刷卡抬杆鉴权
