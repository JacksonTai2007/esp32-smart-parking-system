...
以 ESP32 为主控的智能停车场管理系统：超声波检测入口来车后自动抬杆放行，
SG90 舵机模拟闸机起落，红外传感器实时监测车位占用，OLED 与本地 Web 页面
同步展示剩余车位与系统状态，蜂鸣器提供声音反馈。RFID 刷卡鉴权作为可选模块
保留（`ENABLE_RFID`，默认关闭）。

核心演示流程（默认无刷卡）：

```
车辆靠近入口 → 超声波检测 → 蜂鸣器提示 + SG90 舵机自动开闸 → 欢迎入场
→ 车位传感器更新 → OLED/Web 显示剩余车位 → 闸机到时自动关闸
```

启用 `ENABLE_RFID=1` 后，在开闸前插入刷卡鉴权：检测到车辆 → OLED 显示"请刷卡"
→ 合法卡放行 / 非法卡拒绝。
...
### Phase 1（本期已实现，待硬件实测）
- ✅ 车辆检测自动开闸（默认流程，无需刷卡）
- ✅ RC522 RFID 刷卡识别 + 白名单鉴权（可选模块，`ENABLE_RFID=1` 启用；串口打印每张卡 UID 便于登记）
...
- ✅ 主业务状态机（IDLE → 来车 → 放行入场 / 满位；启用刷卡时插入等待刷卡 → 放行/拒绝）
