...
| 2 | 启动自检 | 上电 | 打印 banner `ESP32 Smart Parking System (slots + billing)`，各模块 `init OK`（Alert/OLED/Slots/Bill/Gate/PM）；OLED 显示 Smart Parking；道闸上电落杆 |
...
| 5 | 车辆入场 | 遮挡某车位（占用） | 识别入场、开始计时，短响 **1 次**，消息 `PN in - timing started`，**道闸抬杆**（`[Gate] OPEN`，OLED `Gate: OPEN`），OLED/Web 该位变占用并显示实时停留时长 |
| 6 | 车辆离场计费 | 移开该车位（空闲） | `[Bill] slot N parked …ms -> fee … cents`，短响 **2 次**，道闸抬杆放行后约 `GATE_OPEN_HOLD_MS` 自动落杆（`[Gate] CLOSE`），OLED/Web 显示本次费用与累计收入，最近记录追加一条；停留 ≤ 免费时长时费用为 0 |
...
| 14 | 道闸手动控制 | 网页点「开闸 / 落闸」 | `POST /api/gate?action=open\|close` → `[Gate] OPEN`/`[Gate] CLOSE`，SG90 抬/落，网页道闸状态与 OLED 同步（`ENABLE_WEB_MANUAL_CONTROL=0` 返回 403） |
| 15 | 模拟模式（可选） | `ENABLE_SIM_MODE=1` 烧录，网页点「驶入/驶离」 | 无需红外即可跑通进出场 / 计费 / 道闸 / 蜂鸣全链路（录视频用），改回 0 恢复真实传感器 |
| 16 | 开关退化编译 | 分别置 `ENABLE_GATE=0`、`ENABLE_SIM_MODE=1` 编译 | 均编译通过（条件编译退化为空实现）|
...
curl -X POST "http://<ESP32-IP>/api/gate?action=open"    # 仅 ENABLE_GATE=1
curl -X POST "http://<ESP32-IP>/api/gate?action=close"
curl -X POST "http://<ESP32-IP>/api/sim?slot=1"          # 仅 ENABLE_SIM_MODE=1
```

`/api/reset` 与 `/api/gate` 受 `ENABLE_WEB_MANUAL_CONTROL` 控制：开关为 1 时返回
`{"ok":true,...}`；为 0 时返回 403、仪表盘只读。`/api/status` 的 `gateEnabled` /
`gateOpen` / `simMode` 字段反映当前编译开关与道闸状态。
