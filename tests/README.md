...
| 2 | 启动自检 | 上电 | 打印 banner `ESP32 Smart Parking System (slots + billing)`，各模块 `init OK`（Alert/OLED/Slots/Bill/Safety/PM）；OLED 显示 Smart Parking |
...
| 8 | 满位提示 | 遮挡全部车位（默认 2 个） | 最后一位占用时长响 **1 次**，消息 `PN in - Parking FULL`，OLED/Web `Free 0/总数` |
...
| 14 | 智能入场分配 | 拍入口触摸片（或网页点「车辆到达入口」） | `[PM] entry: assigned PN`，短响 1 次，网页蓝色引导横幅 + 目标车位高亮，OLED `>> Go to PN <<`；停入后引导消失；满位时长响拒绝（`Entry denied - Parking FULL`） |
| 15 | 模拟模式（可选） | `ENABLE_SIM_MODE=1` 烧录，网页点「车辆到达入口 / 驶入 / 驶离」 | 无需红外即可跑通入场分配 / 进出场 / 计费 / 蜂鸣全链路（录视频用），改回 0 恢复真实传感器 |
| 16 | 开关退化编译 | 分别置 `ENABLE_ENTRY_GUIDE=0`、`ENABLE_FIRE_ALARM=0`、`ENABLE_SIM_MODE=1` 编译 | 均编译通过（条件编译退化为空实现）|
...
curl -X POST "http://<ESP32-IP>/api/entry"               # 仅 ENABLE_ENTRY_GUIDE=1
curl -X POST "http://<ESP32-IP>/api/sim?slot=1"          # 仅 ENABLE_SIM_MODE=1
```

`/api/reset` 与 `/api/entry` 受 `ENABLE_WEB_MANUAL_CONTROL` 控制：开关为 1 时返回
`{"ok":true,...}`；为 0 时返回 403、仪表盘只读。`/api/status` 的 `entryGuide` /
`assignedSlot` / `fireAlarm` / `simMode` 字段反映当前编译开关与运行状态。
